...
class ResNet(nn.Module):
    ...
    def __init__(self, block, list_of_num_layers, list_of_out_dims, flatten=False):
        ...
        self.feat_dim=[]
        self.feat_dim.append([512, 14, 14])
        self.feat_dim.append([512+256, 14, 14])
        self.feat_dim.append([128+512+256, 28, 28])
        # self.feat_dim.append([512, 28, 28])
        # self.feat_dim.append([512, 56, 56])
        self.conv = nn.Sequential(*trunk)
        self.lay1 = nn.Sequential(*layer[0])
        self.lay2 = nn.Sequential(*layer[1])
        self.lay3 = nn.Sequential(*layer[2])
        self.lay4 = nn.Sequential(*layer[3])
        self.dlay4 = nn.Conv2d(512, 256, kernel_size=1)
        self.dlay3 = nn.Conv2d(256, 128, kernel_size=1)
    ...
    def _upsample_add(self, x, y):
        _, _, H, W = y.size()
        p = F.upsample(x, size=(H, W), mode='nearest')
    ...
    def forward(self, x):
        ...
        p4 = self._upsample_add(p5, self.se2(a4))

        p3 = self._upsample_add(p4, self.se3(a3))
...
class resnet(nn.Module):
    ...
    def __init__(self, block, n_blocks, keep_prob=1.0, avg_pool=False, drop_rate=0.0,
        ...
        self.drop_rate = drop_rate
        self.feat_dim=[]
        self.feat_dim.append([640 + 320 + 160, 21, 21])
        self.feat_dim.append([640+320,10,10])
        self.feat_dim.append([640,640,10,10])
        ...
        self.se1 = SELayer(640)
        self.se2 = SELayer(320)
        self.se3 = SELayer(160)
        self.se4 = SELayer(64)
        self.ConvT1 = nn.ConvTranspose2d(960, 960, 2, 1)
    ...
    def forward(self, x, is_feat=False):
        ...
        p3 = torch.cat((self.se1(p4), self.se2(a3)), dim=1)

        p2 = self._upsample_add(p3, self.se3(a2))

        # p1=self._upsample_add(p2,self.se4(a1))
