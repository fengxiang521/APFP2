...
parser.add_argument('--origindataset', default='mini_imagenet', choices=['mini_imagenet', 'cub'])
parser.add_argument('--data_path', type=str,default='/root/autodl-tmp/DataSet/mini-ImageNet')
parser.add_argument('--model', default='ResNet12', choices=['ResNet12', 'ResNet18'])
parser.add_argument('--method', default='meta_deepbdc', choices=['meta_deepbdc', 'protonet', 'good_embed'])
...
parser.add_argument('--targetdataset', default='mini_imagenet', choices=['mini_imagenet', 'tiered_imagenet', 'cub'])

params = parser.parse_args()
num_gpu = set_gpu(params)

json_file_read = False
if params.targetdataset == 'cub':
...
for _ in range(params.test_task_nums):
    ...
    for _, (x, _) in enumerate(tqdm_gen):
        with torch.no_grad():
            ...
            scores,scores2, scores3= model.set_forward(x, False)
        if params.method in ['meta_deepbdc', 'protonet']:
            pred1 = scores.data.cpu().numpy().argmax(axis=1)
        else:
            pred1 = scores
        y = np.repeat(range(params.test_n_way), params.n_query)

        curr1=np.sum(pred1==y)
